...
var pooltoken = document.getElementById('pooltoken')
...
poolform.action = '/pools/' + ((!!pooltoken.textContent || !!pooltoken.innerHTML)
    ? (pooltoken.textContent || pooltoken.innerHTML) + '/post': 'post')
...
new datepickr(dt_start_cal, function (res) {
    dt_start.value = res.length? res[0][0]: '' // timestamp
}, {
...
new datepickr(dt_end_cal, function (res) {
    dt_end.value = res.length? res[0][0]: '' // timestamp
}, {
    singleSelection: true,
