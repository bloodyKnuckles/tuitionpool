...
exports.pools = function (paramsobj, fn) {
  ...
  connection.query(
    ...
    ' CASE `poolname` THEN `poolname` ELSE "untitled" END AS poolname, `pools`.`active`,' +
...
exports.poolsinactive = function (paramsobj, fn) {
  var token = paramsobj.token
  connection.query(
    'SELECT COUNT(*) cnt FROM `pools` WHERE 1 = ? AND active != 1',
    [token],
    resultsThrough(fn, {first:true})
  )
}
...
exports.requestsinactive = function (paramsobj, fn) {
  ...
  connection.query(
    ...
    ' WHERE 1 = ? AND HEX(`pooltoken`) = ? AND active != 1',
    [token, pooltoken],
    resultsThrough(fn, {first:true})
  )
}
