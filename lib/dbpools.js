...
exports.pools = function (paramsobj, fn) {
  ...
  connection.query(
    ...
    ' COUNT(IF(`requests`.`active` = true, 1, NULL)) AS `requestscount`' + // IF(column=3,1,NULL)
...
exports.poolcurr = function (paramsobj, fn) {
  var pooltoken = paramsobj.pooltoken, nowdate = dd.nowdate()
  connection.query(
    ...
    [pooltoken, (paramsobj.startdate && dd.dateFormatEpoch(paramsobj.startdate)) || nowdate, nowdate],
    resultsThrough(fn, {first:true}) // get first record
  )
}

exports.poolnext = function (usertoken, fn) {
  var nowdate = dd.nowdate()
  connection.query(
    ...
    ' FROM `pools` WHERE `datetime_end` >= ? AND `active` = true AND `usertoken` = UNHEX(?)' +
    ' ORDER BY `datetime_start_seconds`',
    [nowdate, nowdate, usertoken],
    //resultsThrough(fn, {first:true}) // get first record
    resultsThrough(fn)
  )
}
...
function requestActivate (activatebool) {
  return function (paramsobj, fn) {
    ...
    connection.query(
      ...
      [activatebool, usertoken, post.pooltoken, post.email, post.datetime],
      resultsThrough(fn)
    )
  }
}
...
exports.requestdelete = function (paramsobj, fn) {
  ...
  connection.query(
    ...
    ' WHERE `usertoken` = UNHEX(?) AND HEX(`pooltoken`) = ? AND `email` = ? AND `datetime` = ?',
    [usertoken, post.pooltoken, post.email, post.datetime],
    resultsThrough(fn)
  )
}
