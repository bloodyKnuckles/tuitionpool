...
exports.requestdeactivate = function (paramsobj, fn) {
    var token = 1, post = paramsobj.post
    connection.query(
        'UPDATE `requests` SET active = false WHERE 1 = ? AND `pool` = ? AND `email` = ? AND `datetime` = ?'
            [
                token, post.pool, post.email, post.datetime
...
exports.requestreactivate = function (paramsobj, fn) {
    var token = 1, post = paramsobj.post
    connection.query(
        'UPDATE `requests` SET active = true WHERE 1 = ? AND `pool` = ? AND `email` = ? AND `datetime` = ?'
            [
                token, post.pool, post.email, post.datetime
...
exports.requestdelete = function (paramsobj, fn) {
    var token = 1, post = paramsobj.post
    connection.query(
        'DELETE FROM `requests` WHERE 1 = ? AND `pool` = ? AND `email` = ? AND `datetime` = ?'
            [
                token, post.pool, post.email, post.datetime
