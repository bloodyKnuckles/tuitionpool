...
exports.pools = function (paramsobj, fn) {
    ...
    connection.query(
        ...
        ' WHERE 1 = ? AND (`requests`.`active` = true OR `requests`.`active` IS NULL)' +
...
exports.requests = function (paramsobj, fn) {
    ...
    connection.query(
        ...
        ' FROM `requests`' +
        ' WHERE 1 = ? AND HEX(`pooltoken`) = ? AND active IN (?,?) ORDER BY `datetime_prov`',
        [token, pooltoken, active1, active2],
        resultsThrough(fn)
    )
}
...
function requestActivate (activatebool) {
    return function (paramsobj, fn) {
        var token = paramsobj.token, post = paramsobj.post
        connection.query(
            'UPDATE `requests` SET active = ' + activatebool +
