...
router.addRoute('/', function (req, res, rmx) {
  ...
  var fileStream = fs.createReadStream('public/home.html')
  fileStream.pipe(res)//*/
})
  ...
  db[rmx.params.itemtype + rmx.params.deletetype]({
    ...
    if ( err || 0 === request.affectedRows ) {
      ...
      templates(
        ...
        {
          '#requestsreturn': {href: '/pools/' + rmx.params.pooltoken},
          '#message': message,
...
router.addRoute('/:statedid(' + statedidpattern + ')', function (req, res, rmx) {
  ...
  function poolNext (usertoken) {
    db.poolnext(usertoken, function (err, poolnext) {
      if ( poolnext && poolnext[0] && poolnext[0].current ) {
        redirect(res, '/' + poolnext[0].pooltoken)
      }
      else {
        //console.log('pool not active')

        var nowepoch = new Date() / 1000
        var provisionallink = poolnext[0] && poolnext[0].datetime_start_seconds - 86400 < Math.floor(nowepoch) &&
          poolnext[0].datetime_end_seconds > nowepoch
          ? {href: poolnext[0].pooltoken + '?provisional'}
          : {style: 'display: none;'},
         message = 'No current pool.' + (poolnext.length? ' Next pool:': '')

        poolnext = (poolnext.length && poolnext.map(function (poolinfo) {
          return {
            '.poolname': poolinfo.poolname,
            '.poolstart': poolinfo.datetime_start,
            '.provstart': poolinfo.datetime_provstart,
            '.poolend': poolinfo.datetime_end,
          }
        })) || [{
            '.poolname': '',
            '.poolstartrow': {style: 'display: none'},
            '.poolendrow': {style: 'display: none'}
          }]
        ...
        templates(
          ...
          {
            '#message': message,
            '#poolslist': {_map: {'.poolinfo': poolnext}}, 
            '#provisionallink': provisionallink
          },
          res
        )
      }
      ...
      db.poolcurr(poolcurrobj, function (err, poolcurr) {
        if ( undefined === poolcurr ) {
          ...
          var provisionallink = poolinfo.datetime_start - 86400 < Math.floor(nowepoch) &&
            ...
            ? {href: rmx.params.pooltoken + '?provisional'}
            : {style: 'display: none;'},
          ...
          templates(
            ['layout.html', 'poolrequestmsg.html'],
            {
              '#message': message,
              '#poolname': poolinfo.poolname,
              '#poolslist': {_map: {'.poolinfo': [{
                '.poolstart': poolinfo.datetime_start_short,
                '.poolend': poolinfo.datetime_end_short,
              }]}},
              '#provisionallink': provisionallink
            },
            res
...
router.addRoute('/login', function (req, res, rmx) {
  ...
  if (
    'POST' === req.method && undefined !== rmx.params &&
    undefined !== rmx.params.username && undefined !== rmx.params.password &&
    '' !== rmx.params.username && '' !== rmx.params.password
  ) {
    db.usercredentials({username:rmx.params.username}, function (err, usercredentialsobj) {
      //console.log('post user credentials', err)
      if ( !err && undefined !== usercredentialsobj ) {
        ...
        login('Error requesting user credentials.')
      }
    })
  }
  else { login() }
...
router.addRoute('/signup', function (req, res, rmx) {

  function signup () {
    message = 'Signup form.'
    templates(
      ['layout.html', 'signup_form.html'],
      {
        '#header': 'Tuition Pool Signup',
        'head': {_mapappend: {
          'link': [{'link':{href:'/css/form.css'}}]
        }},
      },
