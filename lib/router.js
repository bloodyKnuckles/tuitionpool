...
// current/next pool
router.addRoute('/current', function (req, res, rmx) {
  db.poolnext({}, function (err, poolnext) {
...
router.addRoute('/pools', function (req, res, rmx) {
  ...
  db.pools({token:req.session.get('token'), active:active}, function (err, pools) {
    if ( 0 === pools.length ) {
      ...
      db.poolsinactive({token:req.session.get('token')}, function (err, poolsinactive) {
...
router.addRoute('/pools/:pooltoken(' + idpattern + ')', function (req, res, rmx) {
  var queryobj = queryObject(rmx.state.url.split('?')[1]),
    message = 'Starting...',
    active = queryobj && 'false' === queryobj.active? [true,false]: true
  db.poolinfo({token:req.session.get('token'), pooltoken:rmx.params.pooltoken}, function (err, poolinfo) {
    if ( undefined === poolinfo ) {
      message = 'Pool not found.'
      templates(['layout.html', 'poolswrap.html', hscript('div', message)], {}, res)
    }
    else {
      var poolargs = {token:req.session.get('token'), pooltoken:rmx.params.pooltoken, active:active}
      db.requests(poolargs, function (err, requests) {
        if ( 0 === requests.length ) {
          ...
          db.requestsinactive({token:req.session.get('token'), pooltoken:rmx.params.pooltoken}, function (err, requestsinactive) {
...
router.addRoute('/pools/:pooltoken(' + idpattern + ')/post|/pools/post', function (req, res, rmx) {
  var message = 'Starting...'
  if ( !rmx.params.pooltoken ) {
    db.poolinsert({token:req.session.get('token'), post:rmx.params}, function (err, poolinsert) {
    ...
    db.poolinfo({token:req.session.get('token'), pooltoken:rmx.params.pooltoken}, function (err, poolinfo) {
      if ( undefined === poolinfo ) {
        ...
        var poolargs = {token:req.session.get('token'), pooltoken:rmx.params.pooltoken, post:rmx.params}
...
function deleteOptions (req, res, rmx) {
  ...
  db[rmx.params.itemtype + rmx.params.deletetype]({
    token:req.session.get('token'), post:rmx.params
 ...
 console.log('here1', queryobj)
      if ( false === queryobj.active ) {
 console.log('here2')
        db[rmx.params.itemtype + 'sinactive']({token:req.session.get('token'), pooltoken:pooltoken}, function (err, inactive) {
 console.log('here3')
          if ( 0 === inactive.cnt ) {
 console.log('here4')
...
function validateUser(user) {
  var validuser = !!(('jay' === user.uname || 'kathy' === user.uname) && '8244' === user.pword )? true: false
  return {
    'token': '11E64C7EDF998210A6AD9DD86260BC8E',
    'validuser': validuser
  }
}

router.addRoute('/login', function (req, res, rmx) {
  var vu = validateUser({uname: rmx.params.uname, pword: rmx.params.pword})
  if ( 'POST' === req.method && vu.validuser ) {
      //console.log('login post')
      req.session.put('token', vu.token)
      redirect(res, '/pools')
  }
  else {
