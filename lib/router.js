...
function checkUser(req) {
  return undefined !== req.headers.cookie // & 'fun' === cookie(req.headers.cookie).get('token')
}
...
router.addRoute('/pools', function (req, res, rmx) {
  ...
  var activequerystring = queryObject.queryString(queryobj, (true === active? {active:false}: {active:undefined}))
  if ( '' !== activequerystring ) { activequerystring = '?' + activequerystring }
  templates(
    ...
    if ( '' !== activequerystring ) { activequerystring = '?' + activequerystring }
      templates(
        ['layout.html', 'poolswrap.html', 'requests.html'],
        {
          ...
          '#includeactive': {
            href: '/pools/' + rmx.params.pooltoken + activequerystring,
...
router.addRoute('/login', function (req, res, rmx) {
  if ( 'POST' === req.method && validateUser({
      ...
      res.setHeader('Set-Cookie', 'token=fun; expires=' + dd.toUTCString() + '; path=/pools; secure;')
      redirect(res, '/pools')
  }
  else {
