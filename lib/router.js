...
router.addRoute('/pools', function (req, res, rm) {
    var message = 'Starting...',
        active = rm.state.query && 'false' === rm.state.query.active? [true,false]: true
    db.pools({token:1, active:active}, function (err, pools) {
        ...
        templates(
            ['layout.html', 'pools.html'],
            {
                ...
                '#includeactive': {href: '/pools?' + (true === active? 'active=false': ''), _text: (true === active? 'Include': 'Hide') + ' deleted pools' },
                '#scripts': {_mapprepend: {
                    'script': [
...
router.addRoute('/pools/:pooltoken(' + idpattern + ')', function (req, res, rm) {
    ...
    db.poolinfo({token:1, pooltoken:rm.params.pooltoken}, function (err, poolinfo) {
        if ( 0 === poolinfo.length ) {
            ...
            poolinfo.includeactive.href = '/pools/' + rm.params.pooltoken + '?' + (true === active? 'active=false': '')
            poolinfo.includeactive._text = (true === active? 'Include': 'Hide') + ' deleted requests'
            var poolargs = {token:1, pooltoken:rm.params.pooltoken, active:active}
    ...
    function poolrequests (requests, poolinfo) {
        templates(
            ['layout.html', 'poolswrap.html', 'requests.html'],
            {
                ...
                '#requestlink'   : {href: '/' + rm.params.pooltoken, _text: 'http://159.203.135.109/' + rm.params.pooltoken},
                '#listitems': {_map: {'.rowinfo': requests}},
                '#pooltoken': rm.params.pooltoken,
                '#actionform': {action:'/pools/'+ rm.params.pooltoken + '/request/deactivate'},
                '#returnto': {value: '/pools/' + rm.params.pooltoken},
...
function deleteOptions (req, res, rm) {
    ...
    db[rm.params.itemtype + rm.params.deletetype]({
        token:1, post:rm.params
    }, function (err, request) {
        if ( err || 0 === request.affectedRows ) {
            console.log(err, rm.params)
            ...
            templates(
                ['layout.html', 'requestswrap.html', 'poolrequestmsg.html'],
                {
                    ...
                    '#message': message, '#poolname': rm.params.itemtype + ' NOT ' + rm.params.deletetype + 'd'
                }, res
            )
        }
        else if ( 0 < request.affectedRows ) {
            //console.log(rm.params.itemtype + ' ' + rm.params.deletetype) 
            message = rm.params.itemtype + ' ' + rm.params.deletetype
            redirect(res, rm.params.returnto)
        }
    })
}
...
function toggleOptions (req, res, rm) {
    var message = 'Starting...',
        pooltoken = rm.params.pooltoken, email = rm.params.email, datetime = rm.params.datetime
    db[rm.params.columnname + 'toggle']({
        token:1, post:rm.params
    }, function (err, result) {
        if ( err || 0 === result.affectedRows ) {
            console.log(rm.params.columnname, pooltoken, email, datetime, err)
            message = 'Toggle ' + rm.params.columnname + ' failed.'
            templates(
                ['layout.html', 'requestswrap.html', 'poolrequestmsg.html'],
                {
                    '.requests': {href: '/pools/' + rm.params.pooltoken},
                    '#message': message, '#poolname': rm.params.columnname + ' NOT toggled'
                }, res
            )
        }
        else if ( 0 < result.affectedRows ) {
            //console.log(rm.params.deletetype + ' toggle')
            message = rm.params.deletetype + ' toggle'
            redirect(res, '/pools/' + rm.params.pooltoken)
        }
    })
}
