    ...
    /*
      templates(
        ['layout.html', 'request_form.html'],
        {
          ...
          '#poolname': poolnext.poolname, '#pooltoken': rmx.params.pooltoken,
          '#startdate': poolnext.datetime_start,
          '#scripts': {_mapappend: {
      ...
      */
      ...
      var provisionallink = poolnext.datetime_start - 86400 < Math.floor(nowepoch) &&
        poolnext.datetime_end > nowepoch
      ...
      templates(
        ...
        {
          '#message': message,
          '#poolname': poolnext.poolname +
            '<p>Starts: ' + poolnext.datetime_start +
            '<br>Ends: '  + poolnext.datetime_end +
            '</p>' + provisionallink
        },
        res
      )
...
//console.log(req.method, req.url, req.headers.host, req.socket.remotePort, req.socket.localPort, req.headers)
  ...
  db.pools({token:1, active:active}, function (err, pools) {
    if ( 0 === pools.length ) {
      ...
      db.poolsinactive({token:1}, function (err, poolsinactive) {
    ...
    if ( undefined === poolinfo ) {
      ...
      db.requests(poolargs, function (err, requests) {
        if ( 0 === requests.length ) {
          ...
          db.requestsinactive({token:1, pooltoken:rmx.params.pooltoken}, function (err, requestsinactive) {
...
function deleteOptions (req, res, rmx) {
 ...
 console.log('here2')
        db[rmx.params.itemtype + 'sinactive']({token:1, pooltoken:pooltoken}, function (err, inactive) {
 console.log('here3')
          if ( 0 === inactive.cnt ) {
 console.log('here4')
