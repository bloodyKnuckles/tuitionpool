...
// request deactivate
router.addRoute('/pools/:pool(' + idpattern + ')/request/deactivate', function (m, res) {
    var message = 'Starting...'
    db.requestdeactivate({
        token:1, pool:m.params.pool, email:m.params.email, datetime:m.params.datetime
    }, function (requestobj) {
        var request = requestobj.results
        if ( !request[0] ) {
            //console.log('request deactivation failed')
            message = 'Request deactivation failed.'
            templates(
                ['layout.html', 'requestswrap.html', 'poolrequestmsg.html'],
                {
                    '.requests': {href: '/pools/' + m.params.pool},
                    '#message': message
                }, res
            )
        }
...
// request reactivate
router.addRoute('/pools/:pool(' + idpattern + ')/request/reactivate', function (m, res) {
    var message = 'Starting...'
    db.requestreactivate({
        ...
        var request = requestobj.results
        if ( !request[0] ) {
            //console.log('request reactivation failed')
...
// request delete
router.addRoute('/pools/:pool(' + idpattern + ')/request/delete', function (m, res) {
    var message = 'Starting...'
    db.requestdelete({
        token:1, pool:m.params.pool, email:m.params.email, datetime:m.params.datetime
    }, function (requestobj) {
