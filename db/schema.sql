...
CREATE TABLE `pools` (
  `pool` binary(16) NOT NULL,
  `poolname` text,
  `datetime_start` datetime DEFAULT NULL,
  `datetime_end` datetime DEFAULT NULL,
  UNIQUE KEY `pool` (`pool`),
  UNIQUE KEY `pool_2` (`pool`)
...
CREATE TABLE `requests` (
  ...
  KEY `FK_requests` (`pool`),
  CONSTRAINT `FK_requests` FOREIGN KEY (`pool`) REFERENCES `pools` (`pool`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2015-11-17 10:04:25
